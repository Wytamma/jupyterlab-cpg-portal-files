 ...
 */
const NAMESPACE = 'cpg-portal-filebrowser';
console.log(fileIcon.svgstr);
/**
 * The ID for the plugin.
 */
const PLUGIN_ID = 'jupyterlab-cpg-portal-files:plugin';
const COMMAND_ID = 'cpg-portal-files:open';
/**
 * GitLab Icon class.
 */
...
function activateFileBrowser(
  ...
  const tracker = new WidgetTracker<CPGPortalFilesWidget>({
    namespace: 'cpg-portal-files'
  });
  if (restorer) {
    restorer.restore(tracker, {
      command: COMMAND_ID,
      name: () => 'cpg-portal-files'
    });
  }
